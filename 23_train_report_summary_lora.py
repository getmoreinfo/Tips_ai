...
def main() -> None:
    ...
    # A100 40GB 기준: OOM 방지 위해 max_length 2048, batch 2. 학습량은 epochs·grad_accum으로 확보
    ap.add_argument("--max_length", type=int, default=2048, help="최대 시퀀스 길이 (40GB에서는 2048 권장)")
    ap.add_argument("--epochs", type=int, default=10, help="학습 에폭")
    ap.add_argument("--lr", type=float, default=1e-4, help="학습률")
    ap.add_argument("--batch_size", type=int, default=2, help="디바이스당 배치 (OOM 시 1로)")
    ap.add_argument("--grad_accum", type=int, default=16, help="그래디언트 누적 (effective batch = batch_size * grad_accum)")
    ap.add_argument("--lora_r", type=int, default=16, help="LoRA rank (OOM 시 8로)")
    ap.add_argument("--lora_alpha", type=int, default=32, help="LoRA alpha (보통 2*r)")
