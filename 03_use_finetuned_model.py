...
def test_with_training_data(sample_size=100):
    ...
    category_stats = {}
    for i, (pred_cat, confidence) in enumerate(predictions):
        actual_cat = test_df.iloc[i]['category_id']
        cat_name = test_df.iloc[i]['category_name']
    ...
    for cat_id in sorted(category_stats.keys()):
        ...
        print(f"[{cat_id}] {stats['name'][:40]}: {stats['correct']}/{stats['total']} ({cat_accuracy:.1f}%)")
    ...
    for i, (pred_cat, confidence) in enumerate(predictions):
        ...
        if pred_cat != actual_cat:
            errors.append({
                ...
                'actual': actual_cat,
                'actual_name': test_df.iloc[i]['category_name'],
                'predicted': pred_cat,
    ...
    for i, error in enumerate(errors[:10], 1):
        print(f"\n{i}. {error['text']}...")
        print(f"   정답: [{error['actual']}] {error['actual_name'][:40]}")
        print(f"   예측: [{error['predicted']}] (신뢰도: {error['confidence']*100:.1f}%)")
